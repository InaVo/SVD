...
A = []; %some constraints are not used
b = [];
Aeq = [];
beq = [];
...
% definition of functions for lsqnonlin
    ...
    for i = 1:np*ns
        if mod(i,2) == 0
            F(i) = 1-c(y) -(x(l,1)*c(3)+x(l,2)*c(4));
%         elseif mod(i,3) == 0 %only used for three components
%             F(i) = 1-c(y+ns)-c(y) -(x(l,1)*c(7)+x(l,2)*c(8)+x(l,3)*c(9));
            y = y +1;
            l = l+1;
        elseif mod(i,2) == 1
            F(i) = c(y) -(x(l,1)*c(1)+x(l,2)*c(2));
        end
    end
    for i = 1:ns
        F(np*ns+i) = 1- c(ns+np^2+i)-c(np^2+i);
    end
    %additional constraints
    F(np*ns+ns+1) = 0.850 - c(np^2+4);
    F(np*ns+ns+2) = 0.905 - c(np^2+6);
    F(np*ns+ns+3) = 0.74 - c(np^2+5);
    F(np*ns+ns+4) = 1 - c(np^2+7);
    end
